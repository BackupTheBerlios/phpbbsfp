...
echo $result;
	...
	if (count($topic_list)) {
		if ($board_config['load_db_lastread']) {
			...
			$mark_time_forum = (isset($tracking_topics[$forum_id][0])) ? base_convert($tracking_topics[$forum_id][0], 36, 10) + $board_config['board_startdate'] : 0;
		} 
	...
	if ($forum_data['forum_type'] == FORUM_POST && $user['user_id'] != ANONYMOUS && count($topic_list) && $mark_forum_read) {
		markread('mark', $forum_id);
	} 
} 
