...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_smilies.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : admin_smilies.php
...
//
$dir = @opendir($phpbb_root_path . $images['smiles']);

while($file = @readdir($dir))
{
    if( !@is_dir(realpath($phpbb_root_path . $images['smiles'] . '/' . $file)) )
    {
        $img_size = @getimagesize($phpbb_root_path . $images['smiles'] . '/' . $file);
...
if( isset($HTTP_GET_VARS['import_pack']) || isset($HTTP_POST_VARS['import_pack']) )
{
    ...
    if ( !empty($smile_pak) )
    {
        ...
        $fcontents = @file($phpbb_root_path . $images['smiles'] . '/'. $smile_pak);
...
else if( isset($HTTP_POST_VARS['add']) || isset($HTTP_GET_VARS['add']) )
{
    ...
    $template->assign_vars(array(
        ...
        "SMILEY_IMG" => $phpbb_root_path . $images['smiles'] . '/' . $smiley_images[0],

        "S_SMILEY_ACTION" => append_sid("admin_smilies.$phpEx"),
        "S_HIDDEN_FIELDS" => $s_hidden_fields,
        "S_FILENAME_OPTIONS" => $filename_list,
        "S_SMILEY_BASEDIR" => $phpbb_root_path . $images['smiles'])
    );
    ...
    switch( $mode )
    {
        ...
        case 'edit':
            ...
            $template->assign_vars(array(
                ...
                "SMILEY_IMG" => $phpbb_root_path . $images['smiles'] . '/' . $smiley_edit_img,
...
else
{
    ...
    for($i = 0; $i < count($smilies); $i++)
    {
        ...
        $template->assign_block_vars("smiles", array(
            ...
            "SMILEY_IMG" =>  $phpbb_root_path . $images['smiles'] . '/' . $smilies[$i]['smile_url'],
            "CODE" => $smilies[$i]['code'],
