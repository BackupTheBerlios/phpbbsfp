...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_prune_user_posts.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : admin_prune_user_posts.php
...
if( !empty($setmodules) )
{
    ...
    $module['Forums']['Prune_User_Posts'] = append_sid($filename);
    return;
}
...
if( !isset($HTTP_POST_VARS['submit']) )
{
    ...
    $template->set_filenames(array(
        "body" => "admin_prune_user_posts.tpl")
    );
...
else
{
    ...
    if( $topic_ids_sql != '' )
    {
        ...
        if( !$db->sql_query($sql) )
        {
            message_die(CRITICAL_ERROR, 'Could not remove topic view data', '', __LINE__, __FILE__, $sql);
        }
