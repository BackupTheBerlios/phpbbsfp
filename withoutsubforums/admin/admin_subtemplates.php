...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_subtemplates.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : admin_subtemplates.php
...
if( !empty($setmodules) )
{
    ...
    $module['Extreme_Styles']['Subtemplate'] = "$file";
    return;
}
...
if ($mode == 'edit')
{
    ...
    else
    {
        // screen
        $template->set_filenames(array(
            "body" => "admin_subtpl_edit_body.tpl")
        );
        ...
        $dirs = array();
        while ( $file = readdir($main_tpl) ) if ( is_dir($tpl_path . '/' . $file) && !in_array( $file, array('.', '..', 'admin', 'images', 'blocks', 'layout') ) ) $dirs[] = $file;
        closedir($main_tpl);
...
if ($mode == 'stpl')
{
    // screen
    $template->set_filenames(array(
        "body" => "admin_subtpl_list_body.tpl")
    );
...
if ($mode == 'style')
{
    // screen
    $template->set_filenames(array(
        "body" => "admin_subtpl_styles_body.tpl")
    );
