...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_referers.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : admin_referers.php
...
if( !empty($setmodules) )
{
    ...
    $module['General']['HTTP Referers'] = $file;
    return;
}
...
if( isset($HTTP_POST_VARS['delete']) || isset($HTTP_GET_VARS['delete']) )
{
    ...
    {
        message_die(GENERAL_ERROR, "Couldn't delete referrers", '', __LINE__, __FILE__, $sql);
    }
...
else
{
    ...
    if ( $mode == "showurls" )
    {
        ...
        // Count referers
        $sql = "SELECT COUNT(*) AS count FROM " . REFERERS_TABLE;
        ...
        // Query referer info...
        $sql = "SELECT * FROM " . REFERERS_TABLE . " ORDER BY " . $sort_method . " " . $sort_order . " LIMIT ".$start.",".$referers_per_page;
        if(!$result = $db->sql_query($sql))
        {
            message_die(GENERAL_ERROR, "Could not query referers table", "Error", __LINE__, __FILE__, $sql);
        }
    ...
    else
    {
        ...
        // Count referers
        $sql = "SELECT COUNT(DISTINCT referer_host) AS count FROM " . REFERERS_TABLE;
        if(!$result = $db->sql_query($sql))
        {
            message_die(GENERAL_ERROR, "Could not query referers table", "Error", __LINE__, __FILE__, $sql);
        }
        ...
        // Query referer info...
        $sql = "SELECT DISTINCT referer_host, SUM(referer_hits) AS referer_hits, MIN(referer_firstvisit) AS referer_firstvisit, MAX(referer_lastvisit) AS referer_lastvisit FROM " . REFERERS_TABLE . " GROUP BY referer_host ORDER BY " . $sort_method . " " . $sort_order . " LIMIT ".$start.",".$referers_per_page;
        if(!$result = $db->sql_query($sql))
        {
            message_die(GENERAL_ERROR, "Could not query referers table", "Error", __LINE__, __FILE__, $sql);
        }
    ...
    $template->assign_vars(array(
        'PAGINATION' => generate_pagination(append_sid("admin_referers.$phpEx?sort=$sort_method&amp;order=$sort_order" . (isset($mode) ? "&amp;mode=$mode" : "") . ($rdns_ip_num == "" ? "" : "&amp;rdns=$rdns_ip_num")), $total_referers, $referers_per_page, $start),
