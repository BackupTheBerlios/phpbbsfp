...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_board_extend.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
...
if ($submit)
{
	...
	while (list($field_name, $field) = @each($mods[$menu_name]['data'][$mod_name]['data'][$sub_name]['data']))
	{
		...
		{
			// update
			if ($sql = set_config($field_name, $$field_name))
			{
		...
		{
			// update
			if ($sql = set_config($field_name . "_over", intval($HTTP_POST_VARS[$field_name . '_over'])))
			{
				message_die(GENERAL_ERROR, 'Failed to update general configuration for ' . $field_name, '', __LINE__, __FILE__, $sql);
			}
		}
...
$template->set_filenames(array(
	'body' => 'board_config_extend_body.tpl')
);
