...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_forums.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
...
// all the presets
//                View      Read      Post      Reply     Edit     Delete    Sticky   Announce   Vote      Poll       Warn/ban  Unban       Report
...
$return_msg .= '<br /><br />' . sprintf($lang['Click_return_forumadmin'], '<a href="' . append_sid("./admin_forums.$phpEx?selected_id=$selected_id") . '">', '</a>') . '<br /><br />' . sprintf($lang['Click_return_admin_index'], '<a href="' . append_sid("./index.$phpEx?pane=right") . '">', '</a>');
...
if ( ($mode == 'edit') || ($mode == 'create') || ($mode == 'delete') )
{
    ...
    else
    {
        ...
        $template->set_filenames(array(
            'body' => 'forum_extend_edit_body.tpl')
        );
        ...
        $template->assign_vars(array(
            ...
            'U_INDEX'           => append_sid("./admin_forums.$phpEx"),
            'S_ACTION'          => append_sid("./admin_forums.$phpEx"),
            )
        );
    }
}
...
if ( $mode == '' )
{
    ...
    $template->assign_vars(array(
        ...
        'IMG_MOVEUP'        => $phpbb_root_path . $images['DESC_Image'],
        'IMG_MOVEDW'        => $phpbb_root_path . $images['ASC_Image'],
    ...
    for ($i=0; $i < count($tree['sub'][$selected_id]); $i++)
    {
        ...
        for ($j = 0; $j < count($tree['sub'][$this_key]); $j++ )
        {
            ...
            $link = '<span style="white-space: nowrap;"><a href="' . append_sid("./admin_forums.$phpEx?selected_id=$sub_this") . '" class="gensmall" title="' . ereg_replace('<[^>]+>', '', get_object_lang($sub_this, 'desc', true)) . '">';
            $link .= '<img src="' . $sub_folder . '" border="0" alt="' . $sub_l_folder . '" title="' . $sub_l_folder . '" />';
        ...
        $template->assign_block_vars('row', array(
			...
			'U_FORUM'		=> append_sid("./admin_forums.$phpEx?selected_id=$this_key"),
			'U_EDIT'		=> append_sid("./admin_forums.$phpEx?mode=edit&fid=$this_key"),
			'U_DELETE'		=> append_sid("./admin_forums.$phpEx?mode=delete&fid=$this_key"),
			'U_RESYNC'		=> append_sid("./admin_forums.$phpEx?mode=resync&fid=$this_key"),
			'U_MOVEUP'		=> append_sid("./admin_forums.$phpEx?mode=moveup&fid=$this_key"),
			'U_MOVEDW'		=> append_sid("./admin_forums.$phpEx?mode=movedw&fid=$this_key"),
            )
        );
