...
//--------------------------------------------------------------------------------------------------
//
// $Id: xs_edit.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : xs_edit.php
...
if(isset($HTTP_GET_VARS['edit']) && !empty($HTTP_GET_VARS['restore']))
{
	...
	if(@file_exists($backup_name))
	{
		...
		$HTTP_POST_VARS['content'] = addslashes(@file_get_contents($backup_name));
...
if(isset($HTTP_POST_VARS['edit']) && !defined('DEMO_MODE'))
{
	...
	{
		$content = @file_get_contents($HTTP_POST_FILES['upload']['tmp_name']);
	}
	$params = array(
		'edit'		=> $file,
		'dir'		=> $current_dir,
...
if(isset($HTTP_GET_VARS['edit']))
{
	...
	// show template
	$template->set_filenames(array('body' => 'edit_file.tpl'));
...
$template->set_filenames(array('body' => 'edit.tpl'));
$template->pparse('body');
xs_exit();
