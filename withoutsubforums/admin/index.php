...
if(	isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane']	== 'left' )
{
	...
	*/
	...
	$template->assign_vars(array(
		...
		"U_FORUM_INDEX"	=> append_sid("../forum.$phpEx"),
				...
				if (preg_match("~((admin)?[^\/\?&]*)\.$phpEx(?:[\?&]([^<>\s]*))?$~i", $module_url, $matches))
				{
					...
					if ($matches[2]) //	Admin
					{
						...
						foreach	($mvModules	as $name =>	$value)
						{
							...
							{
								$module_url	= $phpbb_root_path . "admin/index.$phpEx?module=$name&file=$matches[1]"	. ($matches[3] ? "&$matches[3]" :	'');
								break;
							}
						}
					}
	...
	if(	count($onlinerow_reg) )
	{
		...
		for($i = 0;	$i < count($onlinerow_reg);	$i++)
		{
			if(	!inarray($onlinerow_reg[$i]['user_id'],	$reg_userid_ary) )
			{
				...
				$page_id = $onlinerow_reg[$i]['user_session_page'];
				if ($page_id < 1)
				{
					switch ($page_id)
					{
						...
						case PAGE_FORUM:
							...
							$location_url =	"../forum.$phpEx?pane=right";
							break;
						case PAGE_POSTING:
						...
						case PAGE_VIEWONLINE:
							...
							$location_url =	"../viewonline.$phpEx?pane=right";
							break;
						case PAGE_VIEWMEMBERS:
							$location =	$lang['Viewing_member_list'];
							$location_url =	"../memberlist.$phpEx?pane=right";
							break;
						case PAGE_PRIVMSGS:
						...
						case PAGE_FAQ:
							...
							$location_url =	"../faq.$phpEx?pane=right";
							break;
						case PAGE_RULES:
...
//--------------------------------------------------------------------------------
				...
				{
					$location_url =	append_sid("admin_forums.$phpEx?mode=editforum&amp;" . POST_FORUM_URL .	"="	. $page_id);
	...
	if(	count($onlinerow_guest)	)
	{
		...
		for($i = 0;	$i < count($onlinerow_guest); $i++)
		{
			...
			$page_id = $onlinerow_guest[$i]['session_page'];
			if ($page_id < 1)
			{
				switch ($page_id)
				{
