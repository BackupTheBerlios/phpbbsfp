 ...
 *                           ----------------------
 *   copyright            : (C) 2003, 2004 Vjacheslav Trushkin
...
$HTTP_POST_VARS['import_default_0']		= non-empty if make it default
...
$file = MV_TEMP_DIR . $filename;
echo $filename;
$header = get_module_header($file);
echo $header;
if($header === false)
{
	echo $filename;
	message_die(GENERAL_ERROR, $lang['xs_style_header_error_reason'] . $xs_header_error . '<br /><br />' . $lang['xs_import_back']);
}
if($header['filesize'] != filesize($file))
{
	...
	message_die(GENERAL_ERROR, $lang['xs_style_header_error_incomplete'] . '<br /><br />' . $lang['xs_import_back']);
}
$f = @fopen($file, 'rb');
if(!$f)
{
	...
	message_die(GENERAL_ERROR, $lang['xs_error_cannot_open'] . '<br /><br />' . $lang['xs_import_back']);
}
if($write_local)
{
...
if($str === false || !strlen($str))
{
	...
	message_die(GENERAL_ERROR, $lang['xs_error_decompress_style'] . '<br /><br />' . $lang['xs_import_back']);
}
//
// unpack tar file
//
			...
			if(!$list_only)
			{
				if($write_local)
				{
					...
					if(!$res)
					{
						...
						message_die(GENERAL_ERROR, str_replace('{FILE}', $save_filename, $lang['xs_error_cannot_create_file']) . '<br /><br />' . $lang['xs_import_back']);
					}
				}
				else
				{
					...
					if(!$f)
					{
						...
						message_die(GENERAL_ERROR, str_replace('{FILE}', $data['tmp'], $lang['xs_error_cannot_create_tmp']) . '<br /><br />' . $lang['xs_import_back']);
					}
					fwrite($f, $contents);
					fclose($f);
				}
...
/*	echo "<!--\n\n";
	...
	if(!$res)
	{
		...
		message_die(GENERAL_ERROR, $ftp_error . '<br /><br />' . $lang['xs_import_back']);
	}
}
...
for($i=0; $i<$total; $i++)
{
	...
	if($tmp)
	{
		$set_default = empty($HTTP_POST_VARS['import_default_'.$i]) ? 0 : 1;
...
if(!$result = $db->sql_query($sql))
{
	...
	message_die(GENERAL_ERROR, $lang['xs_import_notinstall'] . '<br /><br />' . $lang['xs_import_back']);
}
...
if(!@count($data))
{
	...
	message_die(GENERAL_ERROR, $lang['xs_import_notinstall2'] . '<br /><br />' . $lang['xs_import_back']);
}
// install styles
$default_id = 0;
for($i=0; $i<count($install); $i++)
{
	...
	if(empty($style_data['style_name']) || empty($style_data['template_name']))
	{
		...
		message_die(GENERAL_ERROR, str_replace('{STYLE}', $style_name, $lang['xs_import_notinstall3']) . '<br /><br />' . $lang['xs_import_back']);
	}
	if($installed)
	{
		// update
	...
	else
	{
		...
		if ( !($result = $db->sql_query($sql)) )
		{
			...
			message_die(GENERAL_ERROR, $lang['xs_import_notinstall4'] . '<br /><br />' . $lang['xs_import_back']);
		}
	...
	if ( !($result = $db->sql_query($sql)) )
	{
		...
		message_die(GENERAL_ERROR, $lang['xs_import_notinstall5'] . '<br /><br />' . $lang['xs_import_back']);
	}
