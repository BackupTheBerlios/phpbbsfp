...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_users.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : admin_uses.php
...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
    ...
    if ( ( $mode == 'save' && isset( $HTTP_POST_VARS['submit'] ) ) || isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
    {
        ...
        if( $HTTP_POST_VARS['deleteuser'] )
        {
            ...
            $sql = "SELECT instmsgs_id
                FROM " . INSTMSGS_TABLE . "
                WHERE instmsgs_from_userid = $user_id
                    OR instmsgs_to_userid = $user_id";
            if ( !($result = $db->sql_query($sql)) )
            {
			...
			$sql = "SELECT pr.* FROM " . PRIVMSGA_RECIPS_TABLE . " AS pr
                		WHERE pr.privmsg_user_id = " . $user_id;
			if ( !$result = $db->sql_query($sql) )
			{
        ...
        $user_type = ( !empty($HTTP_POST_VARS['usertype']) ) ? trim(strip_tags(htmlspecialchars( $HTTP_POST_VARS['usertype'] ) )) : User_Type_Both;
    ...
    {
        include($phpbb_root_path . 'includes/functions_avatar.'.$phpEx);
        ...
        if( !$error )
        {
            ...
            $sql = "UPDATE " . USERS_TABLE . "
                SET " . $username_sql . $passwd_sql . "user_email = '" . str_replace("\'", "''", $email) . "', user_icq = '" . str_replace("\'", "''", $icq) . "', user_website = '" . str_replace("\'", "''", $website) . "', user_occ = '" . str_replace("\'", "''", $occupation) . "', user_from = '" . str_replace("\'", "''", $location) . "', user_interests = '" . str_replace("\'", "''", $interests) . "', user_sig = '" . str_replace("\'", "''", $signature) . "', user_custom_title = '" . str_replace("\'", "''", $custom_title) . "', user_custom_title_status = $custom_title_status, user_viewemail = $viewemail, user_aim = '" . str_replace("\'", "''", $aim) . "', user_yim = '" . str_replace("\'", "''", $yim) . "', user_msnm = '" . str_replace("\'", "''", $msn) . "', user_attachsig = $attachsig, user_sig_bbcode_uid = '$signature_bbcode_uid', user_allowsmile = $allowsmilies, user_allowhtml = $allowhtml, user_allowavatar = $user_allowavatar, user_allowbbcode = $allowbbcode, user_allow_viewonline = $allowviewonline, user_notify = $notifyreply, user_allow_pm = $user_allowpm, user_notify_pm = $notifypm, user_popup_pm = $popuppm, user_lang = '" . str_replace("\'", "''", $user_lang) . "', user_style = $user_style, user_timezone = $user_timezone, user_dateformat = '" . str_replace("\'", "''", $user_dateformat) . "', user_active = $user_status, user_warnings = $user_ycard, user_rank = $user_rank" . $avatar_sql . " , user_type = $user_type
                WHERE user_id = $user_id";
