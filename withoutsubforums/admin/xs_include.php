...
//--------------------------------------------------------------------------------------------------
//
// $Id: xs_include.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : xs_include.php
...
$template->assign_vars(array(
	'XS_PATH'	=> '../templates/admin/',
	'XS_UL'		=> '<table width="100%" cellspacing="0" cellpadding="2" border="0">',
	'XS_UL2'	=> '</table>',
	'XS_LI'		=> '<tr><td width="20" align="center" valign="middle"><img src="../templates/admin/images/dot.gif" border="0" alt="" /></td><td align="left" valign="middle" width="100%"><span class="gen">',
	'XS_LI2'	=> '</span></td></tr>',
...
if(!defined('NO_XS_HEADER'))
{
	$template->set_filenames(array(
		'xs_header' => 'xs_header.tpl',
		'xs_footer' => 'xs_footer.tpl',
		));
...
//
if(defined('CACHE_THEMES') && @function_exists('cache_themes'))
{
...
function get_ftp_config($action, $post = array(), $allow_local = false, $show_error = '')
{
	...
	if(!empty($HTTP_POST_VARS['get_ftp_config']))
	{
		...
		for($i=0; $i<count($vars); $i++)
		{
			...
			{
				set_config($var, $HTTP_POST_VARS[$var]);
			}
		}
	...
	$template->set_filenames(array('config' => 'ftp.tpl'));
...
function xs_error($error, $line = 0, $file = '')
{
	...
	$template->set_filenames(array('errormsg' => 'message.tpl'));
	$template->assign_vars(array(
...
{
	global $template;
	$template->set_filenames(array('msg' => 'message.tpl'));
	$template->assign_vars(array(
			'MESSAGE_TITLE'	=> $title,
...
function xs_sql($sql, $stip = false)
{
	if($strip)
	{
