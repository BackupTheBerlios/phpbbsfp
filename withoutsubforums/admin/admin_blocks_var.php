...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_blocks_var.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : admin_blocks_var.php
...
if( $mode != "" )
{
    if( $mode == "edit" || $mode == "add" )
    {
        ...
        $template->set_filenames(array(
            "body" => "blocks_variables_edit_body.tpl")
        );
    ...
    else if( $mode == "save" )
    {
        ...
        if( $bv_id )
        {
            $sql = "SELECT config_name FROM " . BLOCKS_VARIABLE_TABLE . " WHERE bvid = $bv_id";
            if(!$result = $db->sql_query($sql))
            {
            ...
            $sql = "UPDATE " . BLOCKS_VARIABLE_TABLE . "
            ...
            $sql2 = "UPDATE " . BLOCKS_CONFIG_TABLE . "
        ...
        else
        {
            ...
            $sql2 = "INSERT INTO " . BLOCKS_CONFIG_TABLE . " (config_name)
    ...
    else if( $mode == "delete" )
    {
        ...
        }else
        {
            if( $bv_id )
            {
                ...
                $sql = "DELETE FROM " . BLOCKS_VARIABLE_TABLE . "
                    WHERE bvid = $bv_id";
                ...
                $sql = "DELETE FROM " . BLOCKS_CONFIG_TABLE . "
...
{
    $template->set_filenames(array(
        "body" => "blocks_variables_list_body.tpl")
    );

    $controltype = array( '1' => 'textbox', '2' => 'dropdown list', '3' => 'radio buttons', '4' => 'checkbox');

    $sql = "SELECT * FROM " . BLOCKS_VARIABLE_TABLE . " ORDER BY block, bvid";
    if( !($result = $db->sql_query($sql)) )
    {
