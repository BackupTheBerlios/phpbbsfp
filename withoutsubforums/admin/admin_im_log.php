...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_im_log.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : admin_im_log.php
...
if( $mode == 'edit' && ( isset($_REQUEST['username']) || isset($_REQUEST[POST_USERS_URL]) ) )
{
    ...
    switch($type)
    {
        case 'offsent':
            ...
            $adminfrom = $userid;
            $sql_user = ' AND pm.instmsgs_from_userid = ' . $userid;
            $msg_sql = $sql_user;
        ...
        case 'offreceived':
            ...
            $adminto = $userid;
            $sql_user = ' AND pm.instmsgs_to_userid = ' . $userid;
            $msg_sql = $sql_user;
            $type_title = $lang['Offsite_Messages_Received_by'] . $username;
            $name_sql = 'pm.instmsgs_from_username, ';
            $offsite = true;
            $network_mark = '&amp;sitesite=1';
            break;
        case 'sent':
            ...
            $adminfrom = $userid;
            $sql_user = ' AND pm.instmsgs_from_userid = ' . $userid;
            $msg_sql = $sql_user . ' AND u.user_id = pm.instmsgs_to_userid';
        ...
        default:
            ...
            $adminto = $userid;
            $sql_user = ' AND pm.instmsgs_to_userid = ' . $userid;
            $msg_sql = $sql_user . ' AND u.user_id = pm.instmsgs_from_userid';
        ...
        else
        {
            ...
            if( $delete && $mark_list )
            {
                ...
                if( !empty($delete_sql_id) )
                {
                    ...
                    $sql = 'SELECT instmsgs_type FROM ' . INSTMSGS_TABLE . ' WHERE instmsgs_id IN (' . $delete_sql_id . ') AND instmsgs_to_userid = ' . $userid;
                    if ( !$result = $db->sql_query($sql) )
                    {
                    ...
                    {
                        if( isset($delete_nums[$val['instmsgs_type']]) )
                        {
                            $delete_nums[$val['instmsgs_type']]++;
                        }
                        else
                        {
                            $delete_nums[$val['instmsgs_type']] = 1;
                        }
                    }

                    $delete_text_sql = 'DELETE FROM ' . INSTMSGS_TEXT_TABLE . ' WHERE instmsgs_text_id IN (' . $delete_sql_id . ')';
                    $delete_sql = 'DELETE FROM ' . INSTMSGS_TABLE . ' WHERE instmsgs_id IN (' . $delete_sql_id . ') AND instmsgs_to_userid = ' . $userid;
    ...
    $sql = 'SELECT pm.instmsgs_type, pm.instmsgs_id, pm.instmsgs_subject, pm.instmsgs_from_userid, ' . $name_sql . 'pm.instmsgs_to_userid, pm.instmsgs_date' . $u_select_sql . '
    FROM ' . INSTMSGS_TABLE . ' pm' . $u_table_sql . '
        WHERE pm.instmsgs_type IN (' . IM_NEW_MAIL . ', ' . IM_UNREAD_MAIL . ', ' . IM_READ_MAIL . ')' . $msg_sql;
    $sql .= ' ORDER BY pm.instmsgs_date ' . $sort_order;
    ...
    $template->set_filenames(array(
        'body' => 'imclient_admin_log_body.tpl')
    );
    ...
    if( $msgs_total )
    {
        ...
        foreach($all_msgs as $key=>$im)
        {
            ...
            {
                case 'offsent':
                    $receiver = $im['instmsgs_to_username'];
                    $u_receiver = $u_profile_base . $im['instmsgs_to_userid'];
                    $adminto = $im['instmsgs_to_userid'];
                    break;
                case 'offreceived':
                    $sender = $im['instmsgs_from_username'];
                    $u_sender = $u_profile_base . $im['instmsgs_from_userid'];
                    $adminfrom = $im['instmsgs_from_userid'];
                    break;
                case 'sent':
            ...
            $template->assign_block_vars('switch_msg_list.switch_msg_row', array(
                'U_IMMSGS' => append_sid(PRILL_URL . '?mode=read&' . POST_POST_URL . '=' . $im['instmsgs_id'] . '&adminmode=1&adminfrom=' . $adminfrom . '&adminto=' . $adminto . $network_mark),
                ...
                'SENDER' => $sender,
                'RECEIVER' => $receiver,
                'DATE' => create_date($board_config['default_dateformat'], $im['instmsgs_date'], $board_config['board_timezone']),
                'IMNUM' => $key,
                'LEFT_PX' => $left_pixels,
                'TOP_PX' => $right_pixels
            ));
