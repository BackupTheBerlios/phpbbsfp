...
//--------------------------------------------------------------------------------------------------
//
// $Id: admin_db_utilities.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME	 : admin_db_utilities.php
...
*	users Minerva database.	 The resulting file should be able to be imported
*	by the db_restore.php function, or by using the mysql command_line
*
...
if (!empty($setmodules))
{
	...
	$module['DB']['DB_optimize'] = $filename . '?perform=optimize';
...
if (isset($HTTP_GET_VARS['perform']) || isset($HTTP_POST_VARS['perform']))
{
	...
	switch ($perform)
	{
		...
		case 'optimize':
			...
			{
				if ($sql = set_config_arr(array('cron_enable', 'cron_every', 'cron_next', 'cron_count', 'show_begin_for', 'show_not_optimized'), array(0, 0, 0, 0, '', 0)))
				{
			...
			if (!isset($HTTP_POST_VARS['optimize']))
			{
				...
				$template->set_filenames(array(
					'body' => 'db_utils_optimize_body.tpl')
				);
				...
				switch ($board_config['cron_every'])
				{
					case 2592000:
						$month = 'selected';
						break;
				...
				intval($board_config['cron_enable']) != 1 ? $enable_cron_no = 'checked="checked"': $enable_cron_yes = 'checked="checked"';
				intval($board_config['show_not_optimized']) != 1 ? $enable_not_optimized_no = 'checked="checked"': $enable_not_optimized_yes = 'checked="checked"';
		
				if ($board_config['cron_enable'] != 1 || $board_config['cron_next'] == 0)
				{
					$next_cron = " - - ";
					$performed_cron = " - - ";
				}
				...
				{
					$next_cron = create_date('d M Y H:i:s', $board_config['cron_next'], $board_config['board_timezone']);
					$performed_cron = $board_config['cron_count'];
				}
				...
				$template->assign_vars(array(
					...
					'S_SHOW_BEGIN_FOR' => $board_config['show_begin_for'],
			...
			else
			{
				...
				// Make optimize query
				if (isset($HTTP_POST_VARS['selected_tbl']) && !empty($HTTP_POST_VARS['selected_tbl']))
				{
					$i=1;
					foreach ($HTTP_POST_VARS['selected_tbl'] as $var => $value)
					{
		...
		case 'backup':
			...
			{
				$gzipcompress = 0;
				if (isset($HTTP_GET_VARS['gzipcompress']) || isset($HTTP_POST_VARS['gzipcompress']))
				{
					$gzipcompress = (isset($HTTP_GET_VARS['gzipcompress'])) ? intval($HTTP_GET_VARS['gzipcompress']) : intval($HTTP_POST_VARS['gzipcompress']);
				}
				$backup_type = '';
				if (isset($HTTP_GET_VARS['backup_type']) || isset($HTTP_POST_VARS['backup_type']))
				{
					$backup_type = (isset($HTTP_GET_VARS['backup_type'])) ? htmlspecialchars($HTTP_GET_VARS['backup_type']) : htmlspecialchars($HTTP_POST_VARS['backup_type']);
				}
				$additional_tables = '';
				if (isset($HTTP_GET_VARS['additional_tables']) || isset($HTTP_POST_VARS['additional_tables']))
				{
					$additional_tables = (isset($HTTP_GET_VARS['additional_tables'])) ? htmlspecialchars($HTTP_GET_VARS['additional_tables']) : htmlspecialchars($HTTP_POST_VARS['additional_tables']);
				}
				if (!isset($HTTP_POST_VARS['startdownload']) && !isset($HTTP_GET_VARS['startdownload']))
				{
					$template->set_filenames(array(
						'body' => 'admin_message_body.tpl')
					);
				...
				$drop = 0;
				if (isset($HTTP_GET_VARS['drop']) || isset($HTTP_POST_VARS['drop']))
				{
					$drop = (isset($HTTP_GET_VARS['drop'])) ? intval($HTTP_GET_VARS['drop']) : intval($HTTP_POST_VARS['drop']);
				}
				...
				{
					preg_match_all("/\s*([^,]+)\s*/", $additional_tables, $found);
					$add_tables = $found[1];
				}
				...
				if (!($result = $db->sql_query($sql)))
				{
					message_die(CRITICAL_ERROR, 'Could not query tables list', '', __LINE__, __FILE__, $sql);
				}
				...
				{
					$table = $row[0];
					if (!strncmp($table, $table_prefix, strlen($table_prefix)) || in_array($table, $add_tables))
					{
						$tables[] = $row[0];
					}
				}
		...
		case 'restore':
			if (!isset($HTTP_POST_VARS['restore_start']))
			{
				...
				$template->set_filenames(array(
					'body' => 'db_utils_restore_body.tpl')
				);
