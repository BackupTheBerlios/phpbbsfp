...
//--------------------------------------------------------------------------------------------------
//
// $Id: search.php,v 1.1 2004/08/30 21:30:06 dmaj007 Exp $
//
// FILENAME	 : search.php
...
*/
							...
							$search_msg_only_card = ($only_bluecards) ? " AND p.post_bluecard > 0 AND m.post_id = p.post_id " : "";
							$sql = "SELECT m.post_id
								FROM " . (($only_bluecards) ? ','.POSTS_TABLE . ' p ' : '') . "" . SEARCH_WORD_TABLE . " w, "	. SEARCH_MATCH_TABLE . " m
								WHERE "	. $search_match	. "
									AND	m.word_id =	w.word_id
									AND	w.word_common <> 1
									$search_msg_only_card";
							// Search Topic	Title Only - End
						}
						else
						{
...
*/
							// Search Topic	Title Only - Start
							$search_match =	( $search_fields ==	0 )	? "pt.post_text LIKE '$match_word'" : ( ( $search_fields == 1) ? "pt.post_text LIKE '$match_word' OR pt.post_subject LIKE '$match_word'" : "pt.post_subject LIKE '$match_word'" );
							$search_match .= ($only_bluecards) ? " AND p.post_bluecard > 0 AND pt.post_id = p.post_id " : "";
...
$template->assign_vars(array(
	...
	'L_TOPIC_STARTER' => $lang['Topic_starter'],
	'L_YES'	=> $lang['Yes'],
	'L_NO' => $lang['No'],
	'L_FORUM' => $lang['Forum'],
	'L_TOPICS' => $lang['Topics'],
