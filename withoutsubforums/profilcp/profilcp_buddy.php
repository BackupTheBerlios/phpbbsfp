...
//--------------------------------------------------------------------------------------------------
//
// $Id: profilcp_buddy.php,v 1.1 2004/08/30 21:30:07 dmaj007 Exp $
//
...
// operators
$operators = array('LE' => $lang['Comp_LE'],
	'EQ' => $lang['Comp_EQ'],
	'NE' => $lang['Comp_NE'],
	'GE' => $lang['Comp_GE'],
	'IN' => $lang['Comp_IN'],
...
if (in_array($set, array('vis', 'inv')))
{
	...
	$sql = 'UPDATE ' . BUDDYS_TABLE . '
		SET buddy_visible = ' . $buddy_visible . '
...
$cm_memberlist->cm_memberlist($user_fields, $user_maps);

if (($set == 'add') && ($buddy_id > 0))
{
	$adduser = true;
}

if (($set == 'remove') && ($buddy_id > 0))
{
	$remove = true;
}
// remove from list
...
else if ($adduser)
{
	...
	{
		$ret_link = $from;
		$template->assign_vars(array('META' => '<meta http-equiv="refresh" content="3;url=' . $ret_link . '">')
			);
		$message = $lang['Profile_updated'] . '<br /><br />' . $l_from . '<br /><br />';
		message_die(GENERAL_MESSAGE, $message);
		exit;
...
else
{
	...
	for ($i = 0, $rank_count = count($ranks); $i < $rank_count; $i++)
	{
	...
	if (!isset($user_fields[$order]))
	{
		$order = 'username';
	}
	if (($order == '') || !in_array($sort, array('ASC', 'DESC')))
	{
		$sort = '';
	}
	if (($order != '') && ($sort == ''))
	...
	if (!isset($user_fields[$filter]))
	{
		$filter = 'username';
	}
	if (!isset($operators[$comp]))
	{
		$comp = 'IN';
	}
	...
	if (!empty($HTTP_POST_VARS['field_ids']) || empty($view_userdata['user_list_option']) || (strlen($view_userdata['user_list_option']) != ($last_ind + 1)))
	{
		...
		for ($i = 0, $maxx = count($field_inds); $i < $maxx; $i++)
		{
			$ind = $field_inds[$i];
			$option[$ind] = '1';
		}
	...
	for ($j = 0; $j <= intval($count / $nb_cell); $j++)
	{
		...
		for ($i = 0; $i < $nb_cell; $i++)
		{
			...
			if ($offset <= $count)
			{
				...
				$template->assign_block_vars('userfields_row.userfields_cell', array('CLASS' => $color ? 'row1' : 'row2',
					'FIELD_ID' => $field_ind,
					'FIELDS' => $field_leg,
	...
	while (list($field_name, $map_field_data) = @each($user_maps[$map_name]['fields'])) {
		if (isset($field_ids[ $map_field_data['ind'] ])) {
			...
			if (($field_name != 'user_email') || is_admin($userdata) || !$board_config['board_email_form']) {
				...
				// fields
				$template->assign_block_vars('header_list', array('L_FIELD' => pcp_output($field_name, $userdata, $map_name, true),
						'ICON_SORT' => $icon_sort,
						)
					);
			}
		...
		for ($i = 0; $i < count($users); $i++) {
			$color = !$color;
			$template->assign_block_vars('userrow', array('CLASS' => ($color) ? 'row1' : 'row2',
					'NUMBER' => $i + 1 + $start,
					)
				);
			@reset($user_maps[$map_name]['fields']);
			...
			if ($friend_list || $ignore_list) {
				$template->assign_block_vars('userrow.select', array('USER_ID' => $users[$i]['user_id'],
						'CHECKED' => '',
						)
					);
			}
		...
		$template->assign_vars(array('L_USER_FIELDS' => $lang['User_fields'],
				'COLSPAN' => $nb_cell,
