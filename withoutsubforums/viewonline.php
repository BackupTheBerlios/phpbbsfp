...
//--------------------------------------------------------------------------------------------------
//
// $Id: viewonline.php,v 1.1 2004/08/30 21:32:17 dmaj007 Exp $
//
...
$layout	= $core_layout[LAYOUT_VIEWONLINE];
...
//-- fin mod : profile cp --------------------------------------------------------------------------
	...
	else
	{
		...
		{
			$BotStr = areyouabot($row['useragent'], FALSE);
      		if (strlen($BotStr) > 0)
      		{
        		$bots_online++;
        		$username = $BotStr;
	...
	if ( $view_online )
	{
		$page_id = $row['session_page'];
		if ($page_id < 1 ||	!$is_auth_ary[$row['session_page']]['auth_view'])
		{
			switch($page_id)
			{
				case PAGE_INDEX:
				...
				default:
					$location =	$lang['Site_index'];
					$location_url =	"index.$phpEx";
					if (isset($mvPages[PAGE_BASE - $page_id]))
					{
						$location =	$lang[$mvPages[PAGE_BASE - $page_id]['name']];
						$location_url =	$mvPages[PAGE_BASE - $page_id]['url'];
					}
			}
		}
		else
		{
			$location_url =	append_sid("viewforum.$phpEx?" . POST_FORUM_URL	. '=' .	$page_id);
			$location =	$forum_data[$page_id];
		}
...
if( $bots_online == 0 )
{
	$l_b_user_s = $lang['bots_zero_online'];
}
else if( $bots_online == 1 )
{
	$l_b_user_s = $lang['bot_online'];
}
else
{
	$l_b_user_s = $lang['bots_online'];
}

$template->assign_vars(array(
	'TOTAL_REGISTERED_USERS_ONLINE'	=> sprintf($l_r_user_s,	$registered_users) . sprintf($l_h_user_s, $hidden_users),
	'TOTAL_GUEST_USERS_ONLINE' => sprintf($l_g_user_s, $guest_users),
	'TOTAL_BOTS_ONLINE' => sprintf($l_b_user_s, $bots_online))
);
