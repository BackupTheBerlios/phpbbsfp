...
//--------------------------------------------------------------------------------------------------
//
// $Id: modcp.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME	 : modcp.php
...
include($phpbb_root_path . 'includes/functions_admin_topic_action_logging.'.$phpEx);
...
preg_match_all('/\s*([0-9]+)\s*/', $GLOBALS['board_config']['no_post_count_forum_id'], $found);
$npcfid = $found[1];
	...
	case 'delete':
		...
		if ( $confirm )
		{
			...
			while ($row = $db->sql_fetchrow($result))
			{
				...
				$topic_titles[intval($row['topic_id'])] = $row['topic_title'];
			}
...
//Auto Group - END
			...
			{
				add_log(LOG_MOD, $forum_id, 0, 'LOG_DELETE', $topic_title);
			}
		...
		if ( $confirm )
		{
			...
			if ( $new_forum_id != $old_forum_id )
			{
				...
				for($i = 0; $i < count($row); $i++)
				{
					...
					// BEGIN Advanced Post Count Mod
					$topic_id_sql .= (($topic_id_sql != '') ? ', ' : '') . intval($topic_id);
					// END Advanced Post Count Mod
					...
					add_log(LOG_MOD, $new_forum_id, $topic_id, 'LOG_MOVE');
				}

				// BEGIN Advanced Post Count Mod
				$posts_sign = '';
				if (in_array($new_forum_id, $npcfid) && !in_array($old_forum_id, $npcfid))
				{
					$posts_sign = '-';
				}
				else if (!in_array($new_forum_id, $npcfid) && in_array($old_forum_id, $npcfid))
				{
					$posts_sign = '+';
				}
				if ($posts_sign != '')
				{
					...
					{
						$count_sql[] = "UPDATE " . USERS_TABLE . " 
							SET user_posts = user_posts $posts_sign " . $row['posts'] . " 
							WHERE user_id = " . $row['poster_id'];
					}
					$db->sql_freeresult($result);
	...
	case 'lock':
		...
		{
			add_log(LOG_MOD, $forum_id, $topic, 'LOG_LOCK');
		}
	...
	case 'unlock':
		...
		foreach ($topics as $topic)
		{
			add_log(LOG_MOD, $forum_id, $topic, 'LOG_UNLOCK');
		}
...
// MOD MODCP EXTENSION BEGIN
	...
	case 'normalise':
		...
		foreach ($topics as $topic)
		{
			add_log(LOG_MOD, $forum_id, $topic, 'LOG_' . strtoupper($mode));
		}
	...
	case 'split':
		...
		if ($post_id_sql != '')
		{
			...
			if ($row = $db->sql_fetchrow($result))
			{
				...
				// BEGIN Advanced Post Count Mod
				$posts_sign = '';
				if (in_array($new_forum_id, $npcfid) && !in_array($forum_id, $npcfid))
				{
					$posts_sign = '-';
				}
				else if (!in_array($new_forum_id, $npcfid) && in_array($forum_id, $npcfid))
				{
					$posts_sign = '+';
				}
				if ($posts_sign != '')
				...
				add_log(LOG_MOD, $new_forum_id, $new_topic_id, 'LOG_SPLIT', array('old_forum_id' => $forum_id, 'old_topic_id' => $topic_id));
