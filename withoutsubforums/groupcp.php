...
//--------------------------------------------------------------------------------------------------
//
// $Id: groupcp.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
...
/*
//$cash_to_points_full_url = ($HTTP_SERVER_VARS['HTTP_HOST'] . $HTTP_SERVER_VARS['REQUEST_URI']);
//$cash_to_points_full_url_dir = str_replace(basename($cash_to_points_full_url), '', $cash_to_points_full_url);
...
$cash_to_points_full_url_dir = $server_protocol	. $server_name . $server_port .	$script_name;

//echo "URL: $cash_to_points_full_url" . " DIR:	$cash_to_points_full_url_dir";die;
*/
$layout	= $core_layout[LAYOUT_USERGROUP];
...
$script_name = preg_replace('/^\/?(.*?)\/?$/', "\\1", trim($board_config['script_path']));
$script_name = ( $script_name != ''	) ?	$script_name . '/%s' : '%s';
$server_name = trim($board_config['server_name']);

$paypal_url	= $server_protocol . $server_name .	$server_port . $script_name;
...
else if	( isset($HTTP_POST_VARS['joingroup']) && $group_id )
{
	...
	if ( $row =	$db->sql_fetchrow($result) )
	{
		...
		if ( $row['group_type']	== GROUP_OPEN || $grouptype	== GROUP_AUTO || $is_autogroup_enable)
		{
			do
			{
...
//-- add
			...
			else
			{
				if ( ( ( isset($HTTP_POST_VARS['approve']) || isset($HTTP_POST_VARS['deny']) ) && isset($HTTP_POST_VARS['pending_members']) )	|| ( isset($HTTP_POST_VARS['remove']) && isset($HTTP_POST_VARS['members']) )	)
				{
					...
					$sql_in	= '';
					for($i = 0,	$m_count = count($members);	$i < $m_count; $i++)
					{
					...
					if ( isset($HTTP_POST_VARS['approve']) )
					{
						...
						for	($i	= 0, $bcc_count	= count($bcc_list);	$i < $bcc_count; $i++)
						{
	...
	$group_info['subscription_id'] = intval($group_info['subscription_id']);
...
//-- modify
	...
	$sql = "SELECT u.username, u.user_id, u.user_viewemail,	u.user_posts, u.user_regdate, u.user_from, u.user_website,	u.user_email, u.user_icq, u.user_aim, u.user_yim, u.user_msnm, ug.user_pending,	ug.group_moderator
...
//-- fin mod : group moderatorZ	--------------------------------------------------------------------
	...
	if ( $group_info['subscription_id'] != 0 )
	{
		// Subscription Based Group; get some details.

		$sql = 'SELECT * FROM ' . GROUPS_SUBSCRIPTIONS_TABLE . '
			WHERE subscription_id = ' . $group_info['subscription_id'];
		...
		$subscription_info = $db->sql_fetchrow($result);

		$db->sql_freeresult($result);

		if ( $subscription_info )
		{
			$template->assign_var('S_IS_PAYPAL_GROUP', TRUE);
		}
		else
		{
			$group_info['subscription_id'] = 0; // Needs thinking
		}
	}
	...
	else if	( $is_group_member || $is_group_pending_member )
	{
		...
		$template->assign_var('S_UNSUBSCRIBE', TRUE);
	...
	{
		if ( $group_info['group_type'] == GROUP_OPEN ) // PayPal - END
		{
			$template->assign_var('S_SUBSCRIBE', TRUE);
		...
		$group_details .= '<br />' . $lang['Group_subscriptions_user_current_fee1'] . ' ' . 
		$subscription_info['subscription_cost'] . $subscription_info['subscription_currency'] . ' ' .
		$lang['Group_subscriptions_user_every']; 

		switch ( $subscription_info['subscription_unit'] )
		{
			case 'D':
				$unit = 'days';
				break;
		...
		$group_details .= ' ' . $subscription_info['subscription_length'] . ' ' . $lang['Group_subscriptions_' . $unit];

		if ($subscription_info['paypal_bgcolor'] == 'B')
		{
			$paypal_bg_color = '<input type=hidden name="cs" value="1" />';
		}
		else
		{
			$paypal_bg_color = '';
...
//-- fin mod : group moderatorZ	--------------------------------------------------------------------
		...
		'L_GROUP_SUBSCRIPTIONS_WARNING'	=> $lang['Group_subscriptions_warning'],
		'L_GROUP_SUBSCRIPTIONS_USER_P2'	=> $subscription_p2,
		'L_GROUP_SUBSCRIPTIONS_USER_FOR_USER_ID' =>	''.$lang['Group_subscriptions_user_for_user_id'].'',
		'PAYPAL_BGCOLOR'	=> $paypal_bg_color,
		'GROUP_SUBSCRIPTIONS_USER_ID' => $userdata['user_id'],
		'PAYPAL_EMAIL'		=> $subscription_info['paypal_email'],
		'PAYPAL_RETURN_URL'	=> append_sid(sprintf($paypal_url, 'index.' . $phpEx)),
		'PAYPAL_NOTIFY_URL'	=> append_sid(sprintf($paypal_url, 'groups_subscriptions_paypal_ipn.' . $phpEx)),
		'PAYPAL_CURRENCY'	=> $subscription_info['subscription_currency'],
		'PAYPAL_COST'		=> $subscription_info['subscription_cost'],
		'PAYPAL_LENGTH'		=> $subscription_info['subscription_length'],
		'PAYPAL_UNIT'		=> $subscription_info['subscription_unit'],
		'PAYPAL_USER_ID'	=> $userdata['user_id'],
...
else
{
	...
	$sql = "SELECT group_id, group_name, group_type, group_count, group_count_max, subscription_id
		FROM " . GROUPS_TABLE .	" g
	...
	$s_group_list_opt = '';
	$s_subscribe_group_list_opt = '';
	while( $row	= $db->sql_fetchrow($result) )
	{
		$is_autogroup_enable = ($row['group_count']	<= $userdata['user_posts'] && $row['group_count_max'] >	$userdata['user_posts']) ? true : false;

		if ( $row['subscription_id'] == 0 && ($row['group_type'] != GROUP_HIDDEN || $userdata['user_level'] == ADMIN || $is_autogroup_enable) )
		{
			$s_group_list_opt .= '<option value="' . $row['group_id'] . '">' . $row['group_name'] . '</option>';
		}
		elseif ( $row['subscription_id'] != 0 && ($row['group_type'] != GROUP_HIDDEN || $userdata['user_level'] == ADMIN) )
		{
			$s_subscribe_group_list_opt .= '<option value="' . $row['group_id'] . '">' . $row['group_name'] . '</option>';
		}
	}

	$s_group_list = '<select name="' . POST_GROUPS_URL . '">' . $s_group_list_opt . '</select>';
	$s_subscribe_group_list = '<select name="' . POST_GROUPS_URL . '">' . $s_subscribe_group_list_opt . '</select>';

	if ( $s_group_list_opt != '' || $s_pending_groups_opt != '' || $s_member_groups_opt != '' || $s_subscribe_group_list_opt != '' )
	{
		//
		...
		if ( $s_subscribe_group_list_opt != '' )
		{
			$template->assign_block_vars('switch_groups_subscriptions',	array()	);
		}
		...
		$template->assign_vars(array(
			...
			'GROUP_LIST_SELECT2' =>	$s_subscribe_group_list,
