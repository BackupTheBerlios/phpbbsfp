...
//--------------------------------------------------------------------------------------------------
//
// $Id: functions_categories_hierarchy.php,v 1.1 2004/08/30 21:30:06 dmaj007 Exp $
//
// FILENAME  : functions_categories_hierarchy.php
...
//function get_auth_keys($cur='Root', $all=false, $level=-1, $max=-1, $auth_key='auth_view')					 // Original Code

function get_auth_keys($cur='Root', $all=false, $level=-1, $max=-1, $auth_key='auth_view', $align_level = 0)	// Bug Fix
{
	global $board_config;
	global $tree;
	...
	if ( ($cur == 'Root') || $tree['auth'][$cur][$auth_key] || $all)
	{
		...
		if (($max < 0) || ($level < $max) || (($level==$max) && ((substr($tree['main'][$tree['keys'][$cur]], 0, 1) == POST_CAT_URL) || ($tree['main'][$tree['keys'][$cur]] == 'Root') )))
		{
			...
			if (!$all)
			{
				// if (($level > 0) && ((substr($cur, 0, 1) == POST_FORUM_URL) || (intval($board_config['sub_forum']) > 0)) && (substr($tree['main'][$tree['keys'][$cur]], 0, 1) == POST_CAT_URL)) $level = $level-1;  // Original Code

				if (($level > 0) && ((substr($cur, 0, 1) == POST_FORUM_URL) || (intval($board_config['sub_forum']) > 0)) && (substr($tree['main'][$tree['keys'][$cur]], 0, 1) == POST_CAT_URL))						   // Bug Fix
				{
					$align_level++;
				}
			...
			{
				$tkeys = array();
				// $tkeys = get_auth_keys($tree['sub'][$cur][$i], $all, $orig_level+1, $max, $auth_key);			  // Original Code
				$tkeys = get_auth_keys($tree['sub'][$cur][$i], $all, $orig_level+1, $max, $auth_key, $align_level);	  // Bug Fix
...
//--------------------------------------------------------------------------------------------------
//
// get_tree_option() : return a drop down menu list of <option></option> - ORIGINAL
//
/*--------------------------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------------------------
//
// get_tree_option() : return a drop down menu list of <option></option> - ENHANCED
//
//--------------------------------------------------------------------------------------------------
function get_tree_option($cur='', $all=false)
{
		...
		if ( ($tree['type'][$this_key] == POST_FORUM_URL) || (($tree['type'][$this_key] == POST_CAT_URL) && !$pull_down))
		{
			...
			if ( $sub && ( !$pull_down || ( ($type == POST_FORUM_URL) && ($sub_forum > 0) ) ) && (intval($board_config['sub_level_links']) > 0) )
			{
				for ($j=0; $j < count($tree['sub'][$cur]); $j++) if ($tree['auth'][ $tree['sub'][$cur][$j] ]['auth_view'])
				{
					...
					if ($link != '') $links .= (($links != '') ? ',</span> ' : '') . $wlast_post . $link;
				}
				if ($links != '')
					$links .= '</span>';
			}
			...
			$template->assign_block_vars('catrow.forumrow', array(
				...
				'U_VIEWFORUM'			=> ($type == POST_FORUM_URL) ? append_sid("viewforum.$phpEx?" . POST_FORUM_URL . "=$id") : append_sid("forum.$phpEx?" . POST_CAT_URL . "=$id"),

//-- mod : new window on forum link ----------------------------------------------------------------
//-- add
				'U_TARGET'				=> ( ($tree['type'][$this] == POST_FORUM_URL) && !empty($tree['data'][$this]['forum_link']) && !$tree['data'][$this]['forum_link_internal']) ? 'target="_blank"' : '',
...
function create_tree()
{
	...
	// build the tree
	$tree = array();
	create_tree_level('Root', $parents, $cats, $forums);
...
function create_tree_level($main, &$parents, &$cats, &$forums, $level=-1)
{
	global $tree;
	...
	{
		$this_c = count($tree['data']);
		$key = $tree_level['type'][$i] . $tree_level['id'][$i];
		$order = $order + 10;
		$tree['keys'][$key] = $this_c;
		$tree['main'][]		= $main;
		...
		create_tree_level($key, $parents, $cats, $forums, $level);
	}
}
