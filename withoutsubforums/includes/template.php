...
/***************************************************************************
 *
 *   eXtreme Styles mod   : 2.0.0
 *
 *   file revision		: 46
 *   project revision	 : 47
 *   last modified		: 22 Jul 2004  19:22:10
 *
 ***************************************************************************/
...
class Template {
	...
	var $xs_started = 0;
	var $xs_version = 4; // number version. internal. do not change.
	var $xs_versiontxt = '2.0.1'; // text version
	...
	/**
	* @return Template
	* @param $root string (Optional)
	* @desc Constructor. Installs XS mod on first run or updates it and sets the root dir.
	*/
	...
	function load_config($root, $edit_db)
	{
		...
		if($edit_db && ((count($add) > 0) || (count($up) > 0) || (count($del) > 0)))
		{
			...
			if(isset($db) && (@get_class($db) === 'sql_db'))
			{
				...
				{
					$sql = "INSERT INTO " . CONFIG_TABLE . " (config_name, config_value) VALUES ('" . $add[$i] . "', '" . str_replace("'", "''", $board_config[$add[$i]]) . "')";
					$db->sql_query($sql);
				}
				...
				{
					$sql = "UPDATE " . CONFIG_TABLE . " SET config_value='" . str_replace("'", "''", $board_config[$up[$i]]) . "' WHERE config_name='" . $up[$i] . "'";
					$db->sql_query($sql);
				}
			}
		}
	...
	* @return boolean
	* @param $dir string
	* @desc Sets the template root directory for this Template object.
	*/
	function set_rootdir($dir)
	{
		...
		if(@file_exists($sub_templates_cfg))
		{
	...
	* @return string $dir
	* @param $dir string
	* @desc Extracts template name from path.
	*/
	...
	* @desc Makes the filename for subtemplates.   
	*/
	function subtemplates_make_filename($filename)
	{
		...
		$sub_templates_cfg = @realpath($this->root . '/sub_templates.cfg');
		...
		if ( !empty($sub_templates) )
		{
			...
			if ( ($forum_id <= 0) && ($cat_id <= 0) )
			{
				if ($post_id > 0)
				{
					...
					if ( $row = $db->sql_fetchrow($result) )
					{
						...
						$HTTP_GET_VARS[POST_FORUM_URL] = $forum_id;
					}
				}
			...
			{
				// get parent level
				$cur = (!empty($tree['main'][ $tree['keys'][$cur] ])) ? $tree['main'][ $tree['keys'][$cur] ] : 'Root';
			...
			if (substr($filename, 0, 1) !== '/')
			{
				...
				for ($i = 0; !$found && ($i < $num_fids); $i++)
				{
					...
					if ( isset($sub_templates[$key]) )
					{
						...
						if (($sub_img_file !== '') && ($sub_tpl_file !== '') && ($sub_css_file !== ''))
						{
							$found = true;
							break;
						}
			...
			for($i = 1; $i < 3; $i++)
			{
				...
				else
				{
					...
					$expvar = !empty($sub_template_key_image) && ($sub_template_key_image !== POST_CAT_URL . '0');
				}
				if ($expvar == 1)
				{
					// get the sub-template path
	...
	{
		global $mvModuleTemplates, $mvModule_root_path;
		...
		if ((substr($filename, 0, 1) !== '/') && (substr($filename, 1, 1) !== ':'))
		{
			...
			{
				$filename = $mvModule_root_path . 'templates/' . $this->tpl . '/' . $filename;
			}
			elseif ($pos = strpos($filename, '_block.tpl') !== false)
			{
				$filename = 'blocks/templates/' . $this->tpl . '/' . substr($filename, $pos-1);
			}
			else
			{
	...
	/**
	* @return boolean
	* @param $filename_array array An array of filenames.
	* @desc Sets the template filenames for handles.
	...
	* @param $handle string
	* @param $filename string
	* @param $xs_include boolean
	* @param $quiet boolean
	* @desc Assigns template filename for handle.
	*/
	...
	* @desc Load the file for the handle, compile the file,
		...
		for($i=0; $i<$line_count; $i++)
		{
			...
			*/
			if($keyword_type == XS_TAG_INCLUDE)
			{
				...
				$line = '<'.'?php ';
				$line .= ' $this->set_filename(\'xs_include' . $this->include_count . '\', \'' . $params_str .'\', true); ';
	...
	* @param $code string The code to be written.
	* @desc Write cache to disk.
	*/
	...
	* @param $name string
	* @param $tpl object	
	* @desc Determines weather or not the size of the block is greater then zero. 
	*/
	...
	// The functions assign_block_var_from_handle(), 
	// reassign_block_vars(), and clear_block_var(),
	// are from classes_cash.php.
	//
	// Dave, remove them if you can find a better
	// way to do the new attach code.
