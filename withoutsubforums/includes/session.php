...
//--------------------------------------------------------------------------------------------------
//
// $Id: session.php,v 1.1 2004/08/30 21:30:06 dmaj007 Exp $
//
...
function session_pagestart($user_ip, $thispage_id)
{
	...
	{
		$thispage_id = constant('MODULE_' .	$mvModuleName .	'_'	. $thispage_id);
	}
	...
	if (preg_match("~((admin)?[^\/\?&]*)\.$phpEx(?:[\?&]([^<>\s]*))?$~i", $url,	$matches))
	{
		...
		if (!empty($matches[2])) //	Admin
		{
			foreach	($mvModules	as $name =>	$value)
			{
				...
				{
					$url = $phpbb_root_path	. "admin/index.$phpEx?module=$name&file=$matches[1]" . ($matches[3]	? "&$matches[3]" : '');
					break;
				}
			}
		...
		{
			$url = "index.$phpEx?module=$mvModuleName&amp;file=$matches[1]"	. (!empty($matches[3]) ? "&amp;$matches[3]"	: '');
		}
	}
