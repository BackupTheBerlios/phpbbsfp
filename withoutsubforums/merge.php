...
//--------------------------------------------------------------------------------------------------
//
// $Id: merge.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME	 : merge.php
...
if ($submit)
{
    ...
    if ($confirm)
    {
        ...
        // grab the topics view to the new topic
        $sql = "UPDATE " . TOPICS_VIEW_TABLE . " SET topic_id=$to_topic_id WHERE topic_id=$from_topic_id" . $sql_user;
        if ( !$db->sql_query($sql) ) message_die(GENERAL_ERROR, 'Could not update topics view table', '', __LINE__, __FILE__, $sql);
        // clean up the old topics watch
        $sql = "DELETE FROM " . TOPICS_VIEW_TABLE . " WHERE topic_id=$from_topic_id";
        if ( !$db->sql_query($sql) ) message_die(GENERAL_ERROR, 'Could not delete topics view table', '', __LINE__, __FILE__, $sql);
        ...
        // update the topic title - BUG FIX!
        if (!empty($topic_title))
        {
        ...
        $mode = "merge";
        add_log(LOG_MOD, $to_forum_id, $to_topic_id, 'LOG_MERGE', array('old_forum_id' => $forum_id, 'old_topic_id' => $topic_id));
