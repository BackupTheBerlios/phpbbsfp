...
//--------------------------------------------------------------------------------------------------
//
// $Id: profile.php,v 1.1 2004/08/30 21:30:06 dmaj007 Exp $
//
...
if ( (isset($HTTP_GET_VARS['mode']) && ($HTTP_GET_VARS['mode'] == 'buddy')) || (isset($HTTP_POST_VARS['mode']) && ($HTTP_POST_VARS['mode'] == 'buddy')) )
{
	define('IN_CASHMOD', true);
...
$layout	= $core_layout[LAYOUT_PROFILE];
...
{
	$view_user_id =	intval($HTTP_POST_VARS[POST_USERS_URL]);
}
...
if ($mode == 'gallery')
{
	include($phpbb_root_path . "profilcp/profilcp_avatar_gallery.$phpEx");
}
if ($mode == 'birthday_popup')
{
	include($phpbb_root_path . "profilcp/profilcp_birthday_popup.$phpEx");
}
if ( !in_array($mode, $module['mode']) ) $mode = 'home';

// no user_id asked, use current user
if ( $userdata['session_logged_in']	&& ($view_user_id == ANONYMOUS)	)
{
	$view_user_id = $userdata['user_id'];
}
...
if ( $userdata['session_logged_in'] && ( ($view_user_id	!= $userdata['user_id']) &&	!is_admin($userdata) &&	($mode != 'activate') ) )
{
	$mode =	'viewprofile';
}
