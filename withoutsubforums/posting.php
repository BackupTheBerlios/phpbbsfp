...
//--------------------------------------------------------------------------------------------------
//
// $Id: posting.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
...
//
$params	= array('submit' =>	'post',	'preview' => 'preview',	'delete' =>	'delete', 'poll_delete'	=> 'poll_delete', 'poll_add' => 'add_poll_option', 'poll_edit' => 'edit_poll_option', 'mode' => 'mode', 'lock_subject' => 'lock_subject' );
...
//
			...
			$sql = "SELECT *
				...
				WHERE vote_id =	$vote_id
					AND	vote_user_id = " . $userdata['user_id']	. $guest_sql;
	...
	switch ( $mode )
	{
		...
		case 'reply':
			...
			if ( $error_msg	== '' )
			{
				...
				if ( $mode == 'editpost' )
				{
					// this should only happen on mod edit...needs to be done ;)
					add_log (LOG_MOD, $forum_id, $topic_id, 'LOG_EDIT');
				}
...
//
// Lock/Unlock topic selection - START
//
...
//
// Lock/Unlock topic selection - END
//
...
if(	$refresh ||	isset($HTTP_POST_VARS['del_poll_option']) || $error_msg	!= '' )
{
    ...
    if( $preview )
    {
        ...
        {
            $user_sig = $bbcode_parse->bbencode_second_pass($user_sig, $userdata['user_sig_bbcode_uid']);
        }
...
// begin Automatic Subject on Reply	mod
		...
		{
			$subject = substr('RE: ' . $subject, 0,	96);
		}
