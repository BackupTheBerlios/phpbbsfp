...
//--------------------------------------------------------------------------------------------------
//
// $Id: im_send.php,v 1.1 2004/08/30 21:30:07 dmaj007 Exp $
//
// FILENAME	 : imclient.php
...
{
    $sql = 'SELECT MAX(instmsgs_date) AS last_post_time
        FROM ' . INSTMSGS_TABLE . '
        WHERE instmsgs_from_userid = ' . $userdata['user_id'];
    if ( $result = $db->sql_query($sql) )
    {
...
if ( $submit && !$error && !$site_id )
{
    ...
    if( $prill_config['enable_im_limit'] )
    {
        ...
        $sql = 'SELECT COUNT(instmsgs_id) AS im_box_items, MIN(instmsgs_date) AS oldest_post_time
            FROM ' . INSTMSGS_TABLE . '
            WHERE (instmsgs_type = ' . IM_UNREAD_MAIL . '
                    OR instmsgs_type = ' . IM_NEW_MAIL . ')
                AND instmsgs_to_userid = ' . $to_userdata['user_id'];
    ...
    $sql_info = 'INSERT INTO ' . INSTMSGS_TABLE . " (site_id, instmsgs_type, instmsgs_subject, instmsgs_from_userid, instmsgs_from_username, instmsgs_to_userid, instmsgs_date, instmsgs_ip, instmsgs_enable_html, instmsgs_enable_bbcode, instmsgs_enable_smilies, instmsgs_attach_sig)
        VALUES ('0', " . IM_NEW_MAIL . ", '" . str_replace("\'", "''", $instant_subject) . "', " . $userdata['user_id'] . ", '', " . $to_userdata['user_id'] . ", $msg_time, '$user_ip', $html_on, $bbcode_on, $smilies_on, $attach_sig)";
    ...
    $sql = 'INSERT INTO ' . INSTMSGS_TEXT_TABLE . " (instmsgs_text_id, instmsgs_bbcode_uid, instmsgs_text)
    ...
    if( $save_sent_im )
    {
        save_a_copy(INSTMSGS_SAVED_OUT_MAIL, $to_userdata, $msg_time, $html_on, $bbcode_on, $smilies_on, $attach_sig, $bbcode_uid, $instant_message, $instant_subject, $site_id);
    }
...
else
{
    ...
    if ( $mode == 'reply' )
    {
        ...
        if( !$site_id )
        {
            $sql = 'SELECT pm.instmsgs_subject, pm.instmsgs_date, pmt.instmsgs_bbcode_uid, pmt.instmsgs_text, u.username, u.user_id
                FROM ' . INSTMSGS_TABLE . ' pm, ' . INSTMSGS_TEXT_TABLE . ' pmt, ' . USERS_TABLE . " u
                WHERE pm.instmsgs_id = $msg_id
                    AND pmt.instmsgs_text_id = pm.instmsgs_id
                    AND pm.instmsgs_to_userid = " . $userdata['user_id'] . '
                    AND u.user_id = pm.instmsgs_from_userid';
        }
        else
        {
            $sql = 'SELECT pm.instmsgs_subject, pm.instmsgs_date, pm.instmsgs_from_userid, pm.instmsgs_from_username, pmt.instmsgs_bbcode_uid, pmt.instmsgs_text
                FROM ' . INSTMSGS_TABLE . ' pm, ' . INSTMSGS_TEXT_TABLE . ' pmt
                WHERE pm.instmsgs_id = ' . $msg_id . '
                    AND pmt.instmsgs_text_id = pm.instmsgs_id
                    AND pm.instmsgs_to_userid = ' . $userdata['user_id'];
        }
        ...
        $instant_subject = ( ( !preg_match('/^Re:/', $im_msg['instmsgs_subject']) ) ? 'Re: ' : '' ) . $im_msg['instmsgs_subject'];
        ...
        {
            $to_userdata['username'] = $im_msg['instmsgs_from_username'];
            $to_userdata['user_id'] = $im_msg['instmsgs_from_userid'];
        }
        ...
        if( $savereply || $saveclose )
        {
            if( $save_a_copy )
            {
                save_a_copy(INSTMSGS_SAVED_IN_MAIL, $to_userdata, $im_msg['instmsgs_date'], $html_on, $bbcode_on, $smilies_on, $attach_sig, $im_msg['instmsgs_bbcode_uid'], addslashes($im_msg['instmsgs_text']), addslashes($im_msg['instmsgs_subject']), $site_id);
            }
