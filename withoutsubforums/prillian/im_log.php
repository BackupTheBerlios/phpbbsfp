...
//--------------------------------------------------------------------------------------------------
//
// $Id: im_log.php,v 1.1 2004/08/30 21:30:07 dmaj007 Exp $
//
// FILENAME	 : im_log.php
...
switch($type)
{
	case 'offsent':
		...
		$sql_user = ' AND pm.instmsgs_from_userid = ' . $userdata['user_id'];
		$msg_sql = $sql_user;
	...
	case 'offreceived':
		...
		$sent_mark = '';
		$sql_user = ' AND pm.instmsgs_to_userid = ' . $userdata['user_id'];
		$msg_sql = $sql_user;
	...
	else
	{
		...
		if( $delete && $mark_list )
		{
			...
			if( !empty($delete_sql_id) )
			{
				...
				$sql = 'SELECT instmsgs_type FROM ' . INSTMSGS_TABLE . ' WHERE instmsgs_id IN (' . $delete_sql_id . ') AND instmsgs_to_userid = ' . $userdata['user_id'];
				...
				$delete_sql = 'DELETE FROM ' . INSTMSGS_TABLE . ' WHERE instmsgs_id IN (' . $delete_sql_id . ') AND instmsgs_to_userid = ' . $userdata['user_id'];
...
$sql = 'SELECT pm.instmsgs_type, pm.instmsgs_id, pm.instmsgs_subject, pm.instmsgs_from_userid, ' . $name_sql . 'pm.instmsgs_to_userid, pm.instmsgs_date' . $u_select_sql . ' FROM ' . INSTMSGS_TABLE . ' pm' . $u_table_sql . '
...
if( $msgs_total )
{
	...
	foreach($all_msgs as $key=>$im)
	{
		...
		$template->assign_block_vars('switch_msg_list.switch_msg_row', array(
			'U_IMMSGS' => append_sid(PRILL_URL . '?mode=read&' . POST_POST_URL . '=' . $im['instmsgs_id'] . $read_mark . $sent_mark . $network_mark),
