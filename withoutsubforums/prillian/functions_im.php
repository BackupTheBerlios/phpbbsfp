...
//--------------------------------------------------------------------------------------------------
//
// $Id: functions_im.php,v 1.1 2004/08/30 21:30:07 dmaj007 Exp $
//
// FILENAME	 : functions_im.php
...
// Saves a copy of an Instant Message in the user's private message Savebox.
{
    ...
    //
    $sql = 'SELECT COUNT(instmsgs_id) AS savebox_items, MIN(instmsgs_date) AS oldest_post_time
        FROM ' . INSTMSGS_TABLE . '
        WHERE ( ( instmsgs_to_userid = ' . $userdata['user_id'] . '
                AND instmsgs_type = ' . INSTMSGS_SAVED_IN_MAIL . ' )
            OR ( instmsgs_from_userid = ' . $userdata['user_id'] . '
                AND instmsgs_type = ' . INSTMSGS_SAVED_OUT_MAIL . ') )';
    if ( !($result = $db->sql_query($sql)) )
    {
    ...
    if ( $saved_info = $db->sql_fetchrow($result) )
    {
        ...
        {
            $sql = 'SELECT instmsgs_id FROM ' . INSTMSGS_TABLE . '
                WHERE ( ( instmsgs_to_userid = ' . $userdata['user_id'] . '
                            AND instmsgs_type = ' . INSTMSGS_SAVED_IN_MAIL . ' )
                        OR ( instmsgs_from_userid = ' . $userdata['user_id'] . '
                            AND instmsgs_type = ' . INSTMSGS_SAVED_OUT_MAIL . ') )
                    AND instmsgs_date = ' . $saved_info['oldest_post_time'];
            if ( !$result = $db->sql_query($sql) )
            {
                message_die(GENERAL_ERROR, 'Could not find oldest instmsgs (save)', '', __LINE__, __FILE__, $sql);
            }
            $old_instmsgs_id = $db->sql_fetchrow($result);
            $old_instmsgs_id = $old_instmsgs_id['instmsgs_id'];

            $sql = "DELETE $sql_priority FROM " . INSTMSGS_TABLE . "
                WHERE instmsgs_id = $old_instmsgs_id";
            if ( !$db->sql_query($sql) )
            {
                message_die(GENERAL_ERROR, 'Could not delete oldest instmsgs (save)', '', __LINE__, __FILE__, $sql);
            }

            $sql = "DELETE $sql_priority FROM " . INSTMSGS_TEXT_TABLE . "
                WHERE instmsgs_text_id = $old_instmsgs_id";
            if ( !$db->sql_query($sql) )
            {
                message_die(GENERAL_ERROR, 'Could not delete oldest instmsgs text (save)', '', __LINE__, __FILE__, $sql);
            }
        }
    }
    ...
    $sql = "INSERT $sql_priority INTO " . INSTMSGS_TABLE . ' (instmsgs_type, instmsgs_subject, instmsgs_from_userid, instmsgs_to_userid, instmsgs_date, instmsgs_ip, instmsgs_enable_html, instmsgs_enable_bbcode, instmsgs_enable_smilies, instmsgs_attach_sig' . $extra_cols . ")
    ...
    $sql = "INSERT $sql_priority INTO " . INSTMSGS_TEXT_TABLE . " (instmsgs_text_id, instmsgs_bbcode_uid, instmsgs_text)
...
function delete_read_ims($user_id)
{
    ...
    $sql = 'SELECT instmsgs_id FROM ' . INSTMSGS_TABLE . ' WHERE instmsgs_type = ' . IM_READ_MAIL . ' AND instmsgs_to_userid = ' . $user_id;
    ...
    foreach($rows as $val)
    {
        $delete_sql_id .= (($delete_sql_id != '') ? ', ' : '') . $val['instmsgs_id'];
    }
    ...
    $delete_sql = 'DELETE FROM ' . INSTMSGS_TABLE . "
        WHERE instmsgs_id IN ($delete_sql_id)
            AND instmsgs_to_userid = " . $user_id;
