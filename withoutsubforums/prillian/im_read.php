...
//--------------------------------------------------------------------------------------------------
//
// $Id: im_read.php,v 1.1 2004/08/30 21:30:07 dmaj007 Exp $
//
// FILENAME	 : im_read.php
...
if( $adminmode )
{
    ...
    if( $network_message )
    {
        $msg_sql = 'AND pm.instmsgs_to_userid = ' . $adminto . ' AND u.user_id = pm.instmsgs_to_userid';
    }
    else
    {
        $msg_sql = 'AND pm.instmsgs_from_userid = ' . $adminfrom . ' AND pm.instmsgs_to_userid = ' . $adminto . ' AND u.user_id = pm.instmsgs_from_userid AND u2.user_id = pm.instmsgs_to_userid';
    }
}
elseif( $sent_mark )
{
    $msg_sql = 'AND pm.instmsgs_from_userid = ' . $userdata['user_id'] . ' AND u.user_id = pm.instmsgs_to_userid AND u2.user_id = pm.instmsgs_from_userid';
}
else
{
    if( $network_message )
    {
        ...
        $msg_sql = 'AND pm.instmsgs_to_userid = ' . $userdata['user_id'] . ' AND u.user_id = pm.instmsgs_to_userid';
    }
    else
    {
        $msg_sql = 'AND pm.instmsgs_to_userid = ' . $userdata['user_id'] . ' AND u.user_id = pm.instmsgs_from_userid AND u2.user_id = pm.instmsgs_to_userid';
    }
}

if( $network_message )
{
    $sql = 'SELECT u.username, u.user_id, pm.*, ss.*, pmt.instmsgs_bbcode_uid, pmt.instmsgs_text FROM ' . INSTMSGS_TABLE . ' pm, ' . INSTMSGS_TEXT_TABLE . ' pmt, ' . USERS_TABLE . ' u, ' . IM_SITES_TABLE . ' ss WHERE pm.instmsgs_id = ' . $msg_id . '   AND pmt.instmsgs_text_id = pm.instmsgs_id ' . $msg_sql . ' AND pm.site_id = ss.site_id';
}
else
{
    $sql = 'SELECT u.username, u.user_id, u.user_sig_bbcode_uid, u.user_sig, pm.*, pmt.instmsgs_bbcode_uid, pmt.instmsgs_text, u2.username AS username2, u2.user_id AS user_id2 FROM ' . INSTMSGS_TABLE . ' pm, ' . INSTMSGS_TEXT_TABLE . ' pmt, ' . USERS_TABLE . ' u, ' . USERS_TABLE . ' u2 WHERE pm.instmsgs_id = ' . $msg_id . '   AND pmt.instmsgs_text_id = pm.instmsgs_id ' . $msg_sql;
}
...
//
if( $mark_read && $im_msg['instmsgs_type'] != IM_READ_MAIL && !$sent_mark )
{
    if( $im_msg['instmsgs_type'] == INSTMSGS_NEW_MAIL || $im_msg['instmsgs_type'] == INSTMSGS_UNREAD_MAIL )
    {
        $type_to_mark = INSTMSGS_READ_MAIL;
    ...
    $sql = 'UPDATE ' . INSTMSGS_TABLE . '
        SET instmsgs_type = ' . $type_to_mark . "
        WHERE instmsgs_id=$msg_id";
...
if( $network_message )
{
    ...
    $u_username_from = append_sid($im_msg['site_url'] . 'profile.' . $im_msg['site_phpex'] . '?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $im_msg['instmsgs_from_userid']);
    $username_from = $im_msg['instmsgs_from_username'];
...
$post_date = create_date($board_config['default_dateformat'], $im_msg['instmsgs_date'], $board_config['board_timezone']);

//
// Processing of post
//
$post_subject = $im_msg['instmsgs_subject'];
if( empty($post_subject) )
{
    $post_subject = $default_im_subject;
}

$instant_message = $im_msg['instmsgs_text'];
$bbcode_uid = $im_msg['instmsgs_bbcode_uid'];

if ( $board_config['allow_sig'] && $im_userdata['attach_sig'] )
{
    $user_sig = ( $im_msg['instmsgs_from_userid'] == $userdata['user_id'] ) ? $userdata['user_sig'] : $im_msg['user_sig'];
}
else
{
    $user_sig = '';
...
$user_sig_bbcode_uid = ( $im_msg['instmsgs_from_userid'] == $userdata['user_id'] ) ? $userdata['user_sig_bbcode_uid'] : $im_msg['user_sig_bbcode_uid'];
...
//
if ( !$board_config['allow_html'] && $im_msg['instmsgs_enable_html'] )
{
    if ( $user_sig != '' && $im_msg['instmsgs_enable_sig'] && $userdata['user_allowhtml'] )
    {
        $user_sig = preg_replace('#(<)([\/]?.*?)(>)#is', "&lt;\\2&gt;", $user_sig);
    }

    if ( $im_msg['instmsgs_enable_html'] )
    {
        $instant_message = preg_replace('#(<)([\/]?.*?)(>)#is', "&lt;\\2&gt;", $instant_message);
    }
}

if ( $user_sig != '' && $im_msg['instmsgs_attach_sig'] && $user_sig_bbcode_uid != '' )
{
...
if ( $bbcode_uid != '' )
{
    $instant_message = ( $board_config['allow_bbcode'] ) ? $bbcode_parse->bbencode_second_pass($instant_message, $bbcode_uid) : preg_replace('/\:[0-9a-z\:]+\]/si', ']', $instant_message);
}

$instant_message = im_make_clickable($instant_message);

if ( $im_msg['instmsgs_attach_sig'] && $user_sig != '' )
{
...
if ( $board_config['allow_smilies'] && $im_msg['instmsgs_enable_smilies'] )
{
    $board_config['smilies_path'] = $phpbb_root_path . $board_config['smilies_path'];
    $instant_message = $bbcode_parse->smilies_pass($instant_message);
}

$instant_message = str_replace("\n", '<br />', $instant_message);

$instant_message = $bbcode_parse->acronym_pass( $instant_message );
$instant_message = $bbcode_parse->smart_pass( $instant_message );
