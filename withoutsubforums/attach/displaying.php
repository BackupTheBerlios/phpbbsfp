...
//--------------------------------------------------------------------------------------------------
//
// $Id: displaying.php,v 1.1 2004/08/30 21:30:05 dmaj007 Exp $
//
// FILENAME  : displaying.php
...
function display_compile_cache_clear($filename, $template_var)
{
    ...
    if (isset($template->cachedir))
    {
        ...
        if (@file_exists(@realpath($template->cachedir . $filename . '.php')))
        {
...
function init_display_template($template_var, $replacement, $filename = 'viewtopic_attach_body.tpl')
{
    ...
    if (!@file_exists(@realpath($complete_filename)))
    {
...
function topic_attachment_image($switch_attachment)
{
    ...
    $image = '<img src="templates/' . $template->tpl . '/images/' . $attach_config['topic_icon'] . '" alt="" border="0" /> ';
...
function display_assign_link($post_id)
{
    ...
    $image = 'templates/' . $template->tpl . '/images/icon_mini_message.gif';
...
{
    global $attach_config, $userdata, $template;
        ...
        for ($i = 0; $i < count($attachment_list); $i++)
        {
            ...
            if (!$denied)
            {
                ...
                if ($link)
                {
                    ...
                    {
                        $upload_image = '<img src="templates/' . $template->tpl . '/images/' . $attach_config['upload_img'] . '" alt="" border="0" />';
                    }
...
//
function display_attachments($post_id, $num_attachments)
{
	...
	$template->set_filename('attachments', 'attachments.tpl');
    //
    // Some basic Template Vars
    //
    $template->assign_vars(array(
    ...
    for ($i = 0; $i < $num_attachments; $i++)
    {
        ...
        if (!$denied)
        {
            ...
            if ( $type != 'image' && $type != 'stream' && $type != 'swf' && $type != 'thumbnail' )
            {
                $type = 'link';
            }
...
//                  'U_DOWNLOAD_LINK' => append_sid('download.' . $phpEx . '?id=' . $attachments['_' . $post_id][$i]['attach_id']),
		...
		$attachrow_vars['TYPE']	= $type;
		$attachrow_vars['L_DENIED'] = ( $denied ) ? (sprintf($lang['Extension_disabled_after_posting'], $attachments['_' . $post_id][$i]['extension'])) : '';
		$template->assign_block_vars('attachrow', $attachrow_vars);
		

	}
	$template->assign_block_var_from_handle('postrow.ATTACHMENTS', 'attachments');
	$template->clear_block_var('attachrow');
}
